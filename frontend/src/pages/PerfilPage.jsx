...
export default function PerfilPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/v1/auth/perfil/${user.id}`, {
        method: 'PUT',
          ...
          <div className={styles.grid}>
            ...
            <div className={styles.infoCard}>
              <div className={styles.avatar}>
                <span className={styles.avatarLetra}>{user.nome?.charAt(0).toUpperCase()}</span>
              </div>
              <p className={styles.infoNome}>{user.nome}</p>
              <p className={styles.infoEmail}>{user.email}</p>
              {user.numero_oab && (
                <div className={styles.infoBadge}>OAB: {user.numero_oab}</div>
              )}
              {user.cpf && (
                <p className={styles.infoDetalhe}>CPF: {user.cpf}</p>
              )}
              <p className={styles.infoDetalhe}>
            ...
            <div className={styles.formCard}>
              <form onSubmit={handleSubmit}>
                ...
                <div className={styles.row}>
                  ...
                  {user.numero_oab && (
                    <div className={styles.field}>
                      ...
                      <input className={styles.input} value={user.numero_oab} disabled />
                    </div>
                  )}
                </div>
                ...
                <div className={styles.senhaHeader}>
                  ...
                  <button type="button" className={styles.btnToggleSenha} onClick={() => setAlterarSenha(!alterarSenha)}>
                    {alterarSenha ? 'Cancelar' : 'Alterar senha'}
                  </button>
                </div>
                ...
                {alterarSenha && (
                  <>
                    <div className={styles.field}>
                      ...
                      <input className={styles.input} type="password" name="senha_atual" value={form.senha_atual} onChange={handleChange} placeholder="••••••••" />
                    </div>
                    <div className={styles.row}>
                      <div className={styles.field}>
                        <label className={styles.label}>Nova senha</label>
                        <input className={styles.input} type="password" name="nova_senha" value={form.nova_senha} onChange={handleChange} placeholder="Mínimo 6 caracteres" />
                      </div>
                      <div className={styles.field}>
                        <label className={styles.label}>Confirmar senha</label>
                        <input className={styles.input} type="password" name="confirmar_senha" value={form.confirmar_senha} onChange={handleChange} placeholder="Repita a senha" />
                      </div>
                    </div>
                  </>
                )}
