...
def determinar_regime_inicial(pena_dias: int, reincidente: bool, natureza: str) -> str:
    """
    Determina o regime inicial conforme art. 33 do Código Penal:
    
    Crimes hediondos e equiparados: sempre regime fechado
    
    Crimes comuns/violentos:
    - Primário:
      - > 8 anos: fechado
      - > 4 anos e <= 8 anos: semiaberto
      - <= 4 anos: aberto
    - Reincidente:
      - > 4 anos: fechado
      - <= 4 anos: semiaberto (juiz pode determinar fechado)
    """
    pena_anos = pena_dias / 360

    # Crimes hediondos e equiparados sempre iniciam em fechado
    if natureza in ('hediondo', 'hediondo_morte', 'feminicidio', 'milicia', 'org_criminosa'):
        return "Fechado"

    if reincidente:
        # Reincidente: > 4 anos vai para fechado
        if pena_anos > 4:
            return "Fechado"
        else:
            return "Semiaberto"
...
def determinar_regime_progressao(regime_atual: str) -> str:
    progressao = {
        "Fechado": "Semiaberto",
        "Semiaberto": "Aberto",
        "Aberto": "Livramento Condicional",
    }
    return progressao.get(regime_atual, "Livramento Condicional")
...
def calcular_execucao(
    ...
    # 4. Regime inicial — corrigido para considerar natureza do crime e reincidência
    regime_inicial = determinar_regime_inicial(pena_base_dias, reincidente, natureza_crime)
